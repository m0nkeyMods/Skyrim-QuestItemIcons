...
using FormOrEditorID = std::variant<FormModPair,   // formID~modName
                                    std::string>;  // editorID
...
struct Range {
    ...
    explicit Range(T a_min) : min(a_min) {}
    Range(T a_min, T a_max) : min(a_min), max(a_max) {}
...
/* inline std::ostream& operator<<(std::ostream& os, RE::TESForm* form) {
    ...
    os << "[" << std::to_string(form->GetFormType()) << ":" << std::setfill('0') << std::setw(sizeof(RE::FormID) * 2)
       << std::uppercase << std::hex << form->GetFormID() << "]";
